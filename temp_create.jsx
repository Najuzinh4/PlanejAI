  ...
  const generateAndSave = async () => {
    ...
    try {
      const { data } = await api.post('/planos', {
        ...
        weekly_focus: weeklyFocus || undefined,
        task_estimated_hours: Number(taskHours) || undefined,\n        weekly_focus: weeklyFocus || undefined,\n        task_estimated_hours: Number(taskHours) || undefined
      });
  ...
  return (
    <div className="container py-6">
      ...
      <div className="grid gap-4">
        ...
        {step === 3 && (
          <Step title="Passo 3: Objetivo e Preferências">
            <div className="grid max-w-xl grid-cols-1 gap-2">
              <input className="rounded-md border px-3 py-2" placeholder="Objetivo (opcional)" value={objetivo} onChange={e => setObjetivo(e.target.value)} />
              <input className="rounded-md border px-3 py-2" placeholder="Foco da semana (opcional)" value={weeklyFocus} onChange={e => setWeeklyFocus(e.target.value)} />
              <input className="rounded-md border px-3 py-2" placeholder="Horas estimadas por tarefa (opcional)" type="number" value={taskHours} onChange={e => setTaskHours(e.target.value)} />
            </div>
            <div className="mt-3 flex gap-2">
        ...
        {step === 4 && (
          <Step title="Passo 4: Gerar com IA → Preview">
            <div className="flex gap-2">
              <button className="rounded-md bg-blue-600 px-4 py-2 text-white disabled:opacity-50" disabled={loading || (selected.length === 0 && subjectNames.length === 0)} onClick={generateAndSave}>{loading ? 'Gerando...' : 'Gerar e Salvar'}</button>
